...
Public Class frm_devolucion_importaciones
    ...
    Const COL_SELLO_EMB As Integer = 25
    ...
    Private Sub frm_devolucion_importaciones_Load(sender As Object, e As EventArgs) Handles MyBase.Load
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    Do While Fila < ds.Tables(0).Rows.Count
                        ...
                        If ds.Tables(0).Rows(Fila)("tdv_codigo") > 0 Then
                            Call CARGA_COMBO_TIPO_DEVOLUCION(Fila)
                        ...
                        GrillaDetalle.Rows(Fila).Cells(COL_SELLO_EMB).Value = ds.Tables(0).Rows(Fila)("emb_sello")
                    ...
                    ''                                ds.Tables(0).Rows(Fila)("dev_fila"),
                    ''                                0,
                    ...
                    ''                                ds.Tables(0).Rows(Fila)("dev_codunico"),
                    ...
                    ''                                0,
                    ''                                ds.Tables(0).Rows(Fila)("dev_cantidad"),
                    ''                                ds.Tables(0).Rows(Fila)("dev_num_bulto"),
                    ''                                ds.Tables(0).Rows(Fila)("dev_total_bulto"),
                    ''                                ds.Tables(0).Rows(Fila)("pro_codigooriginal"),
                    ''                                ds.Tables(0).Rows(Fila)("dev_cant_devuelta"),
                    ''                                ds.Tables(0).Rows(Fila)("dev_cant_bulto"),
                    ''                                ds.Tables(0).Rows(Fila)("dev_motivo"),
                    ''                                ds.Tables(0).Rows(Fila)("pic_fila"),
                    ''                                ds.Tables(0).Rows(Fila)("pdd_fila"),
                    ''                                ds.Tables(0).Rows(Fila)("tdv_codigo"))
                    ''        Fila = Fila + 1
                    ''    Loop
                    ''End If
                End If
    ...
    Private Sub ButtonBuscar_Click(sender As Object, e As EventArgs) Handles ButtonBuscar.Click
        ...
        Dim frm As frm_dev_buscar_prodecto_por_documento = New frm_dev_buscar_prodecto_por_documento
        frm.cnn = _cnn
    ...
    Private Sub GUARDAR_DEVOLUCION()
        ...
        Dim fila As Integer = 0
        Dim _msg As String = ""
        Dim DisponibilizaParaPK As Boolean = 0
        ...
        Try
            ...
            Using Transaccion As New TransactionScope(TransactionScopeOption.Required, scopeOptions)
                ...
                Do While fila <= GrillaDetalle.Rows.Count - 1
                    ...
                    'If GrillaDetalle.Rows(fila).Cells(COL_CANT_DEV).Value < GrillaDetalle.Rows(fila).Cells(COL_CANTIDAD).Value Then
                    '    MessageBox.Show("La cantidad devuelta debe ser menor o igual que la cantidad originl", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                    '    Exit Sub
                    'End If
                    ...
                    'If GrillaDetalle.Rows(fila).Cells(COL_OBSEVACIONES).Value = "" Then
                    classDevolucion.dev_motivo = "-"
                    'Else
                    '    classDevolucion.dev_motivo = GrillaDetalle.Rows(fila).Cells(COL_OBSEVACIONES).Value
                    'End If
                    ...
                    classDevolucion.emb_sello = GrillaDetalle.Rows(fila).Cells(COL_SELLO_EMB).Value
                    ...
                    'VERIFICA EL TIPO DE AGENDA PARA DEJAR O NO PENDIENTE PARA PK
                    DisponibilizaParaPK = 0
                    classDevolucion.tdv_codigo = GrillaDetalle.Rows(fila).Cells(COL_TDV_CODIGO).Value
                    _msgError = ""
                    ds = Nothing
                    ds = classDevolucion.VERIFICA_TIPO_DEVOLUCION(_msgError, conexion)
                    If _msgError = "" Then
                        If ds.Tables.Count > 0 Then
                            If ds.Tables(0).Rows(0)("CODIGO") < 0 Then
                                If conexion.State <> ConnectionState.Closed Then
                                ...
                                Exit Sub
                            Else
                                DisponibilizaParaPK = ds.Tables(0).Rows(0)("CODIGO")
                            End If
                        End If
                    Else
                        If conexion.State <> ConnectionState.Closed Then
                    ...
                    If DisponibilizaParaPK = True Then
                    ...
                    'classDevolucion.pic_codigo = GrillaDetalle.Rows(fila).Cells(COL_N_PICKING).Value
                    'classDevolucion.pic_fila = GrillaDetalle.Rows(fila).Cells(COL_FILA).Value
                    'classDevolucion.observacion = IIf(GrillaDetalle.Rows(fila).Cells(COL_OBSEVACIONES).Value = "", "-", GrillaDetalle.Rows(fila).Cells(COL_OBSEVACIONES).Value)

                    'ds = classDevolucion.MARCA_FILA_DEVUELTA_PICKING(_msgError, conexion)
                    'If ds.Tables(0).Rows.Count > 0 Then
                    '    If _msgError = "" Then
                    '        If ds.Tables(0).Rows(0)("CODIGO") <= 0 Then
                    '            ds = Nothing
                    ...
                    'classDevolucion.sku_cliente = GrillaDetalle.Rows(fila).Cells(COL_SKU_CLIENTE).Value
                    'classDevolucion.dev_codunico = GrillaDetalle.Rows(fila).Cells(COL_COD_UNICO).Value
                    'classDevolucion.dev_cantidad = GrillaDetalle.Rows(fila).Cells(COL_CANT_DEV).Value
                    'ds = classDevolucion.DEVOLUCION_PRODUCTO_ORDEN_CONSOLIDADO(_msgError, conexion)
                    'If ds.Tables(0).Rows.Count > 0 Then
                    '    If _msgError = "" Then
                    '        If ds.Tables(0).Rows(0)("CODIGO") <= 0 Then
                    '            ds = Nothing
    ...
    Private Sub GrillaDetalle_CellEnter(sender As Object, e As DataGridViewCellEventArgs) Handles GrillaDetalle.CellEnter
        Call CARGA_COMBO_TIPO_DEVOLUCION(e.RowIndex)
    End Sub

    Private Sub CARGA_COMBO_TIPO_DEVOLUCION(ByVal fil As Integer)
        ...
        Try
            ...
            _msg = ""
            ds = classTipoDevolucion.TIPO_DEVOLUCION_CARGA_COMBO(_msg)
    ...
    Private Sub GrillaDetalle_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles GrillaDetalle.CellValueChanged
        If GrillaDetalle.Columns(e.ColumnIndex).Name = "DataGridViewTextBoxColumn23" Then
            ...
            Else
                ...
                'GrillaDetalle.Rows(e.RowIndex).Cells(26).Value = obtiene_es_fin_embarque(valor)
            End If
        End If
    End Sub

    'Private Function obtiene_es_fin_embarque(ByVal codigo_tdv As Integer) As Integer
    '    Dim class_devolucion As class_devolucion = New class_devolucion
    '    Dim _msgError As String = ""
    '    Dim ds As DataSet
    '    Dim Fila As Integer = 0
    '    Try
    '        class_devolucion.cnn = _cnn
    '        class_devolucion.tdv_codigo = codigo_tdv

    '        ds = class_devolucion.retorna_fin_embarque(_msgError)
    '        If _msgError = "" Then
    '            If ds.Tables(0).Rows.Count > 0 Then
    '                obtiene_es_fin_embarque = ds.Tables(0).Rows(Fila)("tdv_finEmb")
    '            End If
    '        Else
    '            MessageBox.Show(_msgError + "\obtiene_es_fin_embarque", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
    '        End If
    '    Catch ex As Exception
    '        MessageBox.Show(ex.Message + "\obtiene_es_fin_embarque", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
    '    End Try
    'End Function

    Private Sub GrillaDetalle_CurrentCellDirtyStateChanged(sender As Object, e As EventArgs) Handles GrillaDetalle.CurrentCellDirtyStateChanged
