...
Public Class frm_informe_uni_desp_vs_compra
    ...
    Public Property cnn() As String
        ...
        End Set
    End Property
    Private Sub frm_informe_uni_desp_vs_compra_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Call Inicializa()
    End Sub
    ...
    Private Sub Carga_grilla()
        ...
        Dim diaFechaDesde As String = ""
        Dim mesFechaDesde As String = ""
        Dim mesFechaHasta As String = ""
        Dim diaFechaHasta As String = ""
        Try
            Dim ds As DataSet = New DataSet
            Dim _msg As String
            ...
            If chkDevolucion.Checked = True Then
                If CStr(dtpDevolucionDesde.Value.Month).Length = 1 Then
                    mesFechaDesde = Trim("0" + CStr(dtpDevolucionDesde.Value.Month))
                Else
                    mesFechaDesde = CStr(dtpDevolucionDesde.Value.Month)
                End If

                If CStr(dtpDevolucionDesde.Value.Day).Length = 1 Then
                    diaFechaDesde = Trim("0" + CStr(dtpDevolucionDesde.Value.Day))
                Else
                    diaFechaDesde = CStr(dtpDevolucionDesde.Value.Day)
                End If

                class_informes.str_fechaDesde = CStr(dtpDevolucionDesde.Value.Year) + mesFechaDesde + diaFechaDesde


                'fecha devolucion hasta
                If CStr(dtpDevolucionesHasta.Value.Month).Length = 1 Then
                    mesFechaHasta = Trim("0" + CStr(dtpDevolucionesHasta.Value.Month))
                Else
                    mesFechaHasta = CStr(dtpDevolucionesHasta.Value.Month)
                End If

                If CStr(dtpDevolucionesHasta.Value.Day).Length = 1 Then
                    diaFechaHasta = Trim("0" + CStr(dtpDevolucionesHasta.Value.Day))
                Else
                    diaFechaHasta = CStr(dtpDevolucionesHasta.Value.Day)
                End If
                class_informes.str_fechaHasta = CStr(dtpDevolucionesHasta.Value.Year) + mesFechaHasta + diaFechaHasta
            ...
            _msg = ""
            Grilla.Rows.Clear()
            ds = class_informes.carga_grilla_listado_uni_despachas_vs_compradas(_msg)
            If _msg = "" Then
                    ...
                    If ds.Tables(0).Rows(Fila)("car_codigo") <> "0" Then
                        Do While Fila < ds.Tables(0).Rows.Count
                            Grilla.Rows.Add(ds.Tables(0).Rows(Fila)("car_codigo"),
                                            ...
                                            ds.Tables(0).Rows(Fila)("cant_pedida"),
                                            ds.Tables(0).Rows(Fila)("cant_despac"),
                                            ds.Tables(0).Rows(Fila)("porc_estreg"))
                            Fila = Fila + 1
                        Loop
    ...
    Private Sub ButtonExportar_Click(sender As Object, e As EventArgs) Handles ButtonExportar.Click
        ...
        Try
            ...
            End If
            Cursor = System.Windows.Forms.Cursors.WaitCursor
            Call ExportarDatosExcel(Grilla, "INF0RME UNIDADES DESPACHADAS VS U. COMPRA")
            Cursor = System.Windows.Forms.Cursors.Default
        Catch ex As Exception
